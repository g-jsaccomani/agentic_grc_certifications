...
def test_parse_onboard_txt_content_organization_level_bootstrap_script_output():
    """Verify that the .txt configuration file generated by the organization-level onboard_client.sh bootstrap script parses all metadata accurately."""
    from mcp_server_grc.portal import parse_onboard_txt_content

    script_generated_txt = b"""# GRC Workspace Onboarding Configuration
# Generated automatically by onboard_client.sh
cloud_provider = gcp
client_name = Altostrat Corp
org_id = 123456789012
...
"""
    success, data, err = parse_onboard_txt_content(script_generated_txt, "grc_onboarding_config.txt")
    assert success is True
    assert err is None
    assert data == {
        "cloud_provider": "gcp",
