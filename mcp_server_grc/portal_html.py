...
<head>
    ...
    <style>
        ...
        * {
        ...
        .sidebar.collapsed .client-workspace-wrapper,
        ...
        #cloudProviderSelector .client-dropdown-menu {
            bottom: calc(100% + 6px);
            top: auto;
            max-height: 440px;
            overflow-y: auto;
        ...
        #cloudProviderSelector.client-workspace-wrapper {
            margin-bottom: 8px;
            position: relative;
        }

        #cloudProviderSelector .client-active-card {
            height: auto;
            min-height: unset;
            flex: unset;
        }

        #cloudProviderSelector .provider-dropdown-item {
            display: flex;
            align-items: center;
            gap: 8px;
            padding: 6px 8px;
        ...
        #cloudProviderSelector .provider-dropdown-item:hover {
            background: var(--bg-surface-hover);
            border-color: var(--border-focus);
        }
...
<body>
    ...
    <aside class="sidebar" id="appSidebar">
        ...
        <div class="sidebar-bottom">
            <!-- Conectores Multi-Cloud (Cloud Provider Selector) - compacto no estilo Client Workspace -->
            <div class="client-workspace-wrapper provider-selector-strip-wrapper" id="cloudProviderSelector">
                <div class="client-workspace-header">
                    <div class="client-workspace-title">
                        <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--gcp-blue);">
                ...
                <div class="client-active-card provider-card active" id="provCardGcp" onclick="toggleGcpScopeTree(event)" title="Google Cloud - Conector ativo" data-i18n-title="provider_tooltip_gcp">
                    <div class="client-avatar" id="providerActiveAvatar" style="background: linear-gradient(135deg, #1a73e8, #4285f4);">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 35 28" width="16" height="16" style="vertical-align: middle;"><path fill="#ea4335" d="M21.85,7.41l1,0,2.85-2.85.14-1.21A12.81,12.81,0,0,0,5,9.6a1.55,1.55,0,0,1,1-.06l5.7-.94s.29-.48.44-.45a7.11,7.11,0,0,1,9.73-.74Z"/><path fill="#ffffff" d="M29.76,9.6a12.84,12.84,0,0,0-3.87-6.24l-4,4A7.11,7.11,0,0,1,24.5,13v.71a3.56,3.56,0,1,1,0,7.12H17.38l-.71.72v4.27l.71.71H24.5A9.26,9.26,0,0,0,29.76,9.6Z"/><path fill="#34a853" d="M10.25,26.49h7.12v-5.7H10.25a3.54,3.54,0,0,1-1.47-.32l-1,.31L4.91,23.63l-.25,1A9.21,9.21,0,0,0,10.25,26.49Z"/><path fill="#fbbc05" d="M10.25,8A9.26,9.26,0,0,0,4.66,24.6l4.13-4.13a3.56,3.56,0,1,1,4.71-4.71l4.13-4.13A9.25,9.25,0,0,0,10.25,8Z"/></svg>
                    </div>
                    <div class="client-info">
                        <div class="client-name-row">
                            <span class="client-name" id="providerActiveName">Google Cloud</span>
                            <span class="client-status-pill active" id="providerActiveStatusPill">Active</span>
                        </div>
                        <div class="client-meta-row">
                            <span class="client-projects-count" id="providerActiveProjectsCount">3/10 ativos • Org Conectada</span>
                        </div>
                        <div class="client-expiry-countdown" style="color: var(--gcp-blue);">
                            <svg viewBox="0 0 24 24" width="10" height="10" fill="none" stroke="currentColor" stroke-width="2">
                                <rect x="2" y="7" width="20" height="14" rx="2" ry="2"/>
                                <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/>
                            </svg>
                            <span id="providerActiveOrgName">Altostrat Global Org</span>
                        </div>
                    </div>
                    <svg id="gcpScopeTreeChevron" class="provider-chevron" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" style="transform: rotate(0deg); cursor: pointer;" title="Expandir/Recolher Projetos GCP">
                        <polyline points="6 9 12 15 18 9"/>
                    </svg>
                </div>
                <!-- Dropdown / Pop-up Menu (opens upward) -->
                <div class="client-dropdown-menu" id="providerDropdownMenu">
                    <!-- Multi-cloud Connectors Section -->
                    ...
                    <div class="scope-box" id="scopeContainer" style="border-top: 1px solid var(--border-subtle); padding-top: 8px; margin-top: 4px;">
...
window.currentLanguage = 'pt';
        ...
        function setLanguage(lang) {
            ...
            // Refresh questionnaire if loaded
            if (typeof loadQuestionnaireControls === 'function' && typeof rawQuestionnaireControls !== 'undefined' && rawQuestionnaireControls.length > 0) {
...
Formulário preenchido com o subagente recomendado!`);
        ...
        // Close cloud provider dropdown when clicking outside
        window.addEventListener("click", (e) => {
            const wrapper = document.getElementById("cloudProviderSelector");
            const menu = document.getElementById("providerDropdownMenu");
            const chevron = document.getElementById("gcpScopeTreeChevron");
            if (wrapper && !wrapper.contains(e.target) && menu && menu.classList.contains("active")) {
                menu.classList.remove("active");
...
function openNewsModal(newsKey) {
        ...
        function renderOrgDropdown(filterText = "") {
            ...
            const provCount = document.getElementById("providerActiveProjectsCount");
            if (provCount) {
                provCount.innerText = `${selectedProjectIds.size}/${projectsToDisplay.length} ativos • Org Conectada`;
            }
        }
        ...
        async function loadQuestionnaireControls() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    rawQuestionnaireThemes = data.themes || [];
        ...
        function toggleQuestGuide(safeId) {
            const el = document.getElementById('guideBody_' + safeId);
            const chev = document.getElementById('guideChevron_' + safeId);
            if (!el) return;
            const isHidden = (el.style.display === 'none');
            el.style.display = isHidden ? 'flex' : 'none';
            if (chev) {
                if (isHidden) chev.classList.add('rotated');
        ...
        function renderQuestionnaireAccordion() {
            ...
            const i18nLabels = {
                pt: {
                    ...
                    statusLabel: "Status de Conformidade",
                    ...
                    noControls: "Nenhum controle encontrado com os filtros selecionados.",
                ...
                en: {
                    ...
                    noControls: "No controls found with the selected filters.",
                ...
                es: {
                    ...
                    statusLabel: "Estado de Conformidad",
                    ...
                    saveBtn: "Guardar",
                    controlsCount: "controles",
                    noControls: "No se encontraron controles con los filtros seleccionados.",
                    clickToExpand: "(clic para expandir/plegar)"
                }
            };
            const lbl = i18nLabels[currentLang] || i18nLabels.pt;

            const fallbackThemes = [
                { key: "A.5", title: currentLang === 'en' ? "A.5 — Organizational Controls" : (currentLang === 'es' ? "A.5 — Controles Organizacionales" : "A.5 — Controles Organizacionais"), subtitle: currentLang === 'en' ? "Policies, governance, roles, segregation of duties, and suppliers" : (currentLang === 'es' ? "Políticas, gobernanza, roles, segregación de funciones y proveedores" : "Políticas, governança, papéis, segregação de funções e fornecedores") },
                { key: "A.6", title: currentLang === 'en' ? "A.6 — People Controls" : (currentLang === 'es' ? "A.6 — Controles de Personas" : "A.6 — Controles de Pessoas"), subtitle: currentLang === 'en' ? "Screening, terms of employment, awareness, and offboarding" : (currentLang === 'es' ? "Selección, términos de contratación, concienciación y desvinculación" : "Seleção, termos de contratação, conscientização e desligamento") },
                { key: "A.7", title: currentLang === 'en' ? "A.7 — Physical Controls" : (currentLang === 'es' ? "A.7 — Controles Físicos" : "A.7 — Controles Físicos"), subtitle: currentLang === 'en' ? "Security perimeters, environmental protection, and media disposal" : (currentLang === 'es' ? "Perímetros de seguridad física, protección ambiental y destrucción de soportes" : "Perímetros de segurança, proteção contra desastres e descarte de mídias") },
                { key: "A.8", title: currentLang === 'en' ? "A.8 — Technological Controls" : (currentLang === 'es' ? "A.8 — Controles Tecnológicos" : "A.8 — Controles Tecnológicos"), subtitle: currentLang === 'en' ? "IAM access control, Cloud KMS encryption, VPC protection, and DevSecOps" : (currentLang === 'es' ? "Control de acceso IAM, cifrado Cloud KMS, protección VPC y desarrollo seguro" : "Controle de acesso IAM, criptografia Cloud KMS, proteção VPC e desenvolvimento seguro") }
            ];

            const themesDef = (rawQuestionnaireThemes && rawQuestionnaireThemes.length > 0) ? rawQuestionnaireThemes : fallbackThemes;

            // Filter controls
            const filtered = rawQuestionnaireControls.filter(ctrl => {
                const themeMatch = (currentQuestThemeFilter === 'ALL' || (ctrl.id && ctrl.id.startsWith(currentQuestThemeFilter + '.')) || (ctrl.theme_key === currentQuestThemeFilter));
                const textMatch = !questSearchQuery || (
                    ...
                    (ctrl.question && ctrl.question.toLowerCase().includes(questSearchQuery)) ||
            ...
            if (filtered.length === 0) {
                container.innerHTML = `<div style="text-align:center; padding: 40px; color: var(--text-secondary);">${lbl.noControls}</div>`;
                return;
            }

            let htmlBuf = "";
            themesDef.forEach(thm => {
                const themeControls = filtered.filter(c => (c.id && c.id.startsWith(thm.key + '.')) || (c.theme_key === thm.key));
                if (themeControls.length === 0) return;

                const safeKey = thm.key.replace(/[^a-zA-Z0-9]/g, '_');

                htmlBuf += `
                <div class="quest-theme-card" id="themeCard_${safeKey}">
                    <div class="quest-theme-header open" onclick="toggleQuestAccordion('${safeKey}')">
                        <div class="quest-theme-title">
                            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" class="quest-chevron rotated" id="chevron_${safeKey}">
                                <polyline points="6 9 12 15 18 9"/>
                            </svg>
                            <span>${thm.title}</span>
                            <span class="section-badge">${themeControls.length} ${lbl.controlsCount}</span>
                        </div>
                        <div style="font-size: 11px; color: var(--text-tertiary);">${thm.subtitle || ''}</div>
                    </div>
                    <div class="quest-theme-content" id="content_${safeKey}" style="display: flex; flex-direction: column;">
                `;
                ...
                themeControls.forEach(ctrl => {
                    ...
                    const ctrlName = tr.name || ctrl.name || ctrl.id;
                    const ctrlQuestion = tr.question || ctrl.question || `A organização cumpre os requisitos do controle ${ctrl.id}?`;
                    const ctrlDesc = tr.description || ctrl.description || '';
                    const ctrlEvidence = tr.recommended_evidence || ctrl.recommended_evidence || '';
                    const ctrlCheck = ctrl.how_to_check || '';
                    const ctrlMapping = ctrl.gcp_mapping || '';

                    const safeId = ctrl.id.replace(/[^a-zA-Z0-9]/g, '_');
                    ...
                    <div class="quest-control-card" id="card_${ctrl.id}">
                        ...
                        <div class="quest-callout-box">
                            <div class="quest-callout-header">
                                ...
                                <span>${lbl.auditQuestion}</span>
                            </div>
                            <p class="quest-callout-text">${escapeHtml(ctrlQuestion)}</p>
                        </div>

                        <!-- Scope and Requirement -->
                        ${ctrlDesc ? `
                        <div style="font-size: 12px; color: var(--text-secondary); line-height: 1.45; margin: 2px 0 6px 0;">
                        ...
                        <div class="quest-evidence-guide">
                            <div class="quest-guide-header" onclick="toggleQuestGuide('${safeId}')">
                                ...
                                <span style="flex: 1;">${lbl.evidenceGuide}</span>
                                <span style="font-size: 10.5px; opacity: 0.7; margin-right: 6px;">${lbl.clickToExpand}</span>
                                <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" id="guideChevron_${safeId}" class="quest-chevron rotated"><polyline points="6 9 12 15 18 9"/></svg>
                            </div>
                            <div class="quest-guide-body" id="guideBody_${safeId}" style="display: flex;">
                                ${ctrlEvidence ? `
                                <div class="guide-item">
                                    <span class="guide-label">${lbl.evidenceArtifacts}</span>
                                    <pre class="guide-content">${escapeHtml(ctrlEvidence)}</pre>
                                </div>` : ''}
                                ${ctrlCheck ? `
                                <div class="guide-item">
                                    <span class="guide-label">${lbl.gcpCheck}</span>
                                    <pre class="guide-content">${escapeHtml(ctrlCheck)}</pre>
                                </div>` : ''}
                                ${ctrlMapping ? `
                                <div class="guide-item" style="font-size: 11px; color: var(--text-tertiary); margin-top: 2px;">
                                    <strong style="color: var(--text-secondary);">${lbl.gcpTelemetry}</strong> ${escapeHtml(ctrlMapping)}
                                </div>` : ''}
                            </div>
                        </div>
                        ...
                        <div class="quest-control-form">
                            <div class="quest-form-field">
                                <label for="status_${ctrl.id}" class="quest-form-label">${lbl.statusLabel}</label>
                            ...
                            <div class="quest-form-field">
                                <label for="just_${ctrl.id}" class="quest-form-label">${lbl.justLabel}</label>
                                <textarea id="just_${ctrl.id}" class="quest-textarea" placeholder="${lbl.justPlaceholder}">${escapeHtml(justVal)}</textarea>
                            </div>
                            <div class="quest-form-field">
                                <label for="uri_${ctrl.id}" class="quest-form-label">${lbl.uriLabel}</label>
                                <input type="text" id="uri_${ctrl.id}" class="quest-input" placeholder="${lbl.uriPlaceholder}" value="${escapeHtml(uriVal)}">
