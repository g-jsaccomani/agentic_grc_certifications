...
async def run_phased_audit(
    ...
    # Attach evidence links directly into the scoped Continuous Intelligence engine
    for item in scan_results:
        cid = item.get("control_id")
        if cid:
            try:
                scoped_engine.evidence_graph.add_link(
                    control_id=cid,
                    framework="ISO27001:2022",
                    ...
                    verification_tier=item.get("verification_tier", "TELEMETRY"),
                )
            except Exception as e:
                logger.debug(f"Link addition error for {cid}: {e}")

    # Track FinOps usage for this client's scan execution
    scoped_finops = get_client_finops_tracker(target_cid)
    scoped_finops.record_usage(
        agent_id="gcp-telemetry",
        prompt_tokens=1420 * len(projects),
    ...
    if is_demo and scoped_finops is not finops_tracker:
